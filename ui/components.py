        ...
        <div class="header-content">
            <div class="logo-section">
                <div class="logo-animation">
                    ...
                    <span class="logo-text">🔒 CodexSafe</span>
                </div>
                <div class="tagline">
                    <span class="highlight">Explainable Security Platform</span>
                    <span class="separator">•</span>
                    <span class="org">Turning Code Into Confidence</span>
                </div>
            </div>
...
def display_features():
    """Display CodexSafe AI security capabilities with world-class, futuristic design"""

    # Capability Matrix
    st.markdown(
        """
        <div class="capability-matrix">
            <div class="capability-node security-node">
                <div class="node-core">
                    <div class="core-pulse"></div>
                    <div class="core-label">SECURITY</div>
                </div>
                <div class="node-description">
                    <h3>Challenge Mode & Analyze Mode</h3>
                    <p>Interactive security training and code analysis</p>
                </div>
    ...
    st.markdown(
        ...
        <div class="processing-pipeline">
            <div class="pipeline-stage stage-analysis">
                <div class="stage-core">ANALYZE</div>
                <div class="stage-description">Code Analysis</div>
            </div>
            <div class="pipeline-flow">
                <div class="flow-particle"></div>
            </div>
            <div class="pipeline-stage stage-detection">
                <div class="stage-core">DETECT</div>
                <div class="stage-description">Vulnerability Detection</div>
            </div>
            <div class="pipeline-flow">
                <div class="flow-particle"></div>
            </div>
            <div class="pipeline-stage stage-fix">
                <div class="stage-core">FIX</div>
                <div class="stage-description">Auto-Fix & Suggestions</div>
            </div>
            <div class="pipeline-flow">
                <div class="flow-particle"></div>
            </div>
            <div class="pipeline-stage stage-validate">
                <div class="stage-core">VALIDATE</div>
                <div class="stage-description">Security Validation</div>
            </div>
        </div>
    """,
        unsafe_allow_html=True,
...
def sidebar_control_panel() -> Dict[str, Any]:
    ...
    """
    with st.sidebar:
        st.markdown("### 🔒 CodexSafe Control Panel")

        # Application status
        if st.session_state.processing:
            st.warning("🟡 Security Analysis Processing...")
        else:
            st.info("⚪ Security Engine Ready")
        ...
        st.markdown(f"### Current Mode: {mode_icons.get(current_mode, '❓')} {current_mode.title()}")

        # Workflow configuration options
        st.markdown("### ⚙️ Security Settings")

        # Indexing functionality toggle (for generate mode)
        enable_indexing = st.checkbox(
            "🗂️ Enable Full Analysis",
            value=True,
            help="Enable comprehensive code analysis and indexing. Disable for faster processing.",
            key="enable_indexing",
        )
...
def footer_component():
    ...
    <div style="text-align: center; color: #666; padding: 2rem;">
        <p>🔒 <strong>CodexSafe</strong> | Explainable Security Platform | Turning Code Into Confidence |
        <a href="https://github.com/SiriYellu/CodexSafe" target="_blank" style="color: var(--neon-blue);">GitHub</a></p>
        <p>⚡ Challenge Mode • AI Explainer • Real Vulnerability Detection • Game Mode Practice</p>
        <p><small>💡 Master secure coding through interactive challenges and AI-powered explanations</small></p>
    </div>
    """,
        unsafe_allow_html=True,
    )
...
def display_main_header():
    """Display the main CodexSafe header with title"""
    st.markdown("""
    <div style="text-align: center; margin: 2rem 0;">
        <h1 style="font-size: 4rem; color: #ffffff; margin: 0; display: flex; align-items: center; justify-content: center; gap: 1rem;">
            🔒 CodexSafe
        </h1>
        <p style="font-size: 1.2rem; color: #a0a0a0; margin: 1rem 0;">
            Explainable AI Security Platform - Understand Every Security Decision
        </p>
    </div>
    """, unsafe_allow_html=True)

def display_feature_badges():
    """Display feature badges row"""
    st.markdown("""
    <div style="text-align: center; margin: 1.5rem 0; display: flex; flex-wrap: wrap; justify-content: center; gap: 0.5rem;">
        <span style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 0.5rem 1rem; border-radius: 20px; font-size: 0.9rem; margin: 0.2rem;">🎮 Game Mode</span>
        <span style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 0.5rem 1rem; border-radius: 20px; font-size: 0.9rem; margin: 0.2rem;">🎯 Exploit Simulator</span>
        <span style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 0.5rem 1rem; border-radius: 20px; font-size: 0.9rem; margin: 0.2rem;">💡 AI Explainer</span>
        <span style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 0.5rem 1rem; border-radius: 20px; font-size: 0.9rem; margin: 0.2rem;">👑 Real Breaches</span>
        <span style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 0.5rem 1rem; border-radius: 20px; font-size: 0.9rem; margin: 0.2rem;">🛡️ OWASP Linked</span>
        <span style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 0.5rem 1rem; border-radius: 20px; font-size: 0.9rem; margin: 0.2rem;">🌐 6 Languages</span>
        <span style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 0.5rem 1rem; border-radius: 20px; font-size: 0.9rem; margin: 0.2rem;">🐛 9 Vulnerabilities</span>
    </div>
    <div style="text-align: center; margin: 0.5rem 0;">
        <span style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 0.5rem 1rem; border-radius: 20px; font-size: 0.9rem;">📄 Reports</span>
    </div>
    """, unsafe_allow_html=True)

def display_pro_tip():
    """Display pro tip banner"""
    st.markdown("""
    <div style="background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); color: white; padding: 1rem; border-radius: 12px; margin: 1rem 0; text-align: center;">
        <strong>💡 Pro Tip:</strong> Enable <strong>Game Mode</strong> to test your skills! Write your own fixes and get scored 0-100. Master secure coding by practicing, not just watching!
    </div>
    """, unsafe_allow_html=True)

def display_choose_your_mode():
    """Display the four main mode buttons"""
    ...
    with col1:
        ...
        with col1_1:
            if st.button("🏠 Home", use_container_width=True, help="Main dashboard and overview"):
                st.session_state['current_mode'] = 'home'
                st.rerun()
        with col1_2:
            if st.button("🚀 Generate Code", use_container_width=True, help="Generate new secure code from requirements"):
    ...
    with col2:
        ...
        with col2_1:
            if st.button("🔍 Analyze Code", use_container_width=True, help="Analyze existing code for security issues"):
                st.session_state['current_mode'] = 'analyze'
                st.rerun()
        with col2_2:
            if st.button("🎯 Challenge Mode", use_container_width=True, help="Practice with vulnerable code challenges"):
                st.session_state['current_mode'] = 'challenge'
                st.rerun()

def display_challenge_mode():
    """Display Challenge Mode interface"""
    st.markdown("## 🎯 Challenge Mode - Game Mode")
    ...
    with col2:
        framework = st.selectbox(
            "Select Framework:",
            ["Flask", "Express", "Spring", ".NET", "Gin", "Laravel"],
            key="challenge_framework"
        )
    
    # Vulnerability Type Selection
    vulnerability = st.selectbox(
        "Select Vulnerability to Practice:",
        ["SQL Injection", "XSS (Cross-Site Scripting)", "Hardcoded Secrets", 
         "Command Injection", "Path Traversal", "CSRF", "Authentication Bypass"],
        key="challenge_vulnerability"
    )
    
    # Generate Challenge Button
    if st.button("🎯 Generate Vulnerable Code Challenge", type="primary", use_container_width=True):
        st.session_state['challenge_generated'] = True
        
        # Sample vulnerable code based on selection
        if vulnerability == "SQL Injection" and language == "Python":
            vulnerable_code = '''
...
'''
            ...
            vulnerable_code = f'''
# Sample vulnerable code for {vulnerability} in {language}
def vulnerable_function():
    # TODO: Implement {vulnerability} vulnerability
    pass
'''
        
        st.session_state['vulnerable_code'] = vulnerable_code
        st.session_state['selected_challenge_language'] = language
        st.session_state['selected_challenge_framework'] = framework
        st.session_state['selected_challenge_vulnerability'] = vulnerability

def display_analyze_mode():
    """Display Analyze Code interface"""
    st.markdown("## 🔍 Analyze Code - Security Review")
    ...
    if st.button("🔍 Start Security Analysis", type="primary", use_container_width=True):
        if code_to_analyze and len(code_to_analyze.strip()) > 10:
            st.success("✅ Analysis complete!")
            st.markdown("### 📊 Analysis Results")
            st.metric("Security Score", "85/100")
        else:
            st.error("Please provide code to analyze!")


def display_simple_generate_interface():
    """Display simplified Generate Code interface for CodexSafe"""
    st.markdown("## Generate Secure Code")
    st.markdown("Create secure code from your requirements with built-in vulnerability scanning.")
    
    # Simple text input for requirements
    st.markdown("### Enter Your Requirements")
    requirements = st.text_area(
        "Describe what you want to build:",
        height=150,
        placeholder="Example: Create a secure login system for a web application",
        key="generate_requirements"
    )
    ...
    with col1:
        language = st.selectbox(
            "Select Language:",
            ["Python", "JavaScript", "Java", "C#", "Go", "PHP"],
            key="generate_language"
        )
    
    with col2:
        framework = st.selectbox(
            "Select Framework:",
            ["Flask", "Express", "Spring", ".NET", "Gin", "Laravel", "Django", "React"],
            key="generate_framework"
        )
    
    # Generate button
    if st.button("Generate Secure Code", type="primary", use_container_width=True):
        if requirements and len(requirements.strip()) > 10:
            st.success("✅ Code generation started!")
            
            # Show a simple result (placeholder for now)
...
def secure_handler():
    ...
    # Includes input validation, error handling, and security headers
    pass
""", language="python")
            
            st.info("🔒 Security features: Input validation, SQL injection protection, XSS prevention")
        else:
            st.error("Please provide detailed requirements to generate code!")
