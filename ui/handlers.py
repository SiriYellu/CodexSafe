...
def run_async_task(coro):
    ...
    try:
        executor = concurrent.futures.ThreadPoolExecutor(
            max_workers=1, thread_name_prefix="codexsafe_ctx_async"
        )
...
def run_async_task_simple(coro):
    ...
    try:
        ...
        if loop.is_running():
            ...
            try:
                executor = concurrent.futures.ThreadPoolExecutor(
                    max_workers=1, thread_name_prefix="codexsafe_async"
                )
...
def initialize_session_state():
    ...
    # CodexSafe specific session state initialization
    ...
    if "vulnerable_code" not in st.session_state:
        st.session_state.vulnerable_code = ""
    if "selected_challenge_language" not in st.session_state:
        st.session_state.selected_challenge_language = ""
    if "selected_challenge_framework" not in st.session_state:
        st.session_state.selected_challenge_framework = ""
    if "selected_challenge_vulnerability" not in st.session_state:
        st.session_state.selected_challenge_vulnerability = ""
